...
| `tool-graphify` | ✅ **instalada** (también global en `~/.claude/skills/` — aplica a TODOS los proyectos). diga "graphify", "mapea/indexa este proyecto", "monta el grafo de conocimiento", "grafo de dependencias", o pregunte arquitectura ("¿qué depende de X?") en un repo grande. Always-on vía passive rule Sinapsis `graphify-graph-first` (modo **query-first**: usa `graphify query/explain/path`; NUNCA leas `GRAPH_REPORT.md`/`graph.json` enteros — son enormes y agotan el contexto). NUNCA usar `graphify claude install`. Cada repo tiene su grafo propio en `graphify-out/` |
