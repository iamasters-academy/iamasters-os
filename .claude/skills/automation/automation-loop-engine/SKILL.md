...
description: >
  Motor de Loop Engineering: convierte trabajo repetitivo en sistemas que se diseñan UNA vez y se ejecutan N veces con verificación, compuertas humanas y aprendizaje acumulado. Tres funciones: INSTALACIÓN (entrevista de 5 preguntas al primer uso), RADAR (detecta cuándo una tarea se vuelve loop) y FÁBRICA (diseña, dispara, opera y evalúa el loop). Úsala cuando el operador diga "loop", "sistematiza esto", "automatiza este proceso", "en lote", "todas las semanas", "para cada cliente", "deja de hacerlo a mano", "convierte esto en un sistema", o pida repetir una tarea hecha 2+ veces. Activa proactivamente (RADAR) a la 3ª repetición de una misma tarea, aunque no lo pida.
