...
description: >
  Diseña y mejora ofertas y el encuadre de valor: bundling, garantías, bonuses, urgencia, framing de
  la propuesta. Úsala cuando el operador diga "diseña una oferta", "mejora mi oferta", "cómo empaqueto
  esto", "qué incluyo en el paquete", "value framing", "haz esto más irresistible". Encaja con los
  servicios propios (Informe de Jugador, consultora). NO la uses para FIJAR el precio en sí
  ([[marketing-pricing]]) ni para el plan global ([[marketing-plan]]).
version: 0.1.0
---
...
> `context/`. "Related Skills" por nombre pelado (`pricing`, `paywalls`…) son aquí `marketing-<nombre>`.
