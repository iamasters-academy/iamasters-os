...
name: marketing-plan
description: >
  Desarrolla un plan de marketing completo: objetivos, canales, presupuesto, calendario y KPIs, a
  partir del contexto de producto. Úsala cuando el operador diga "hazme un plan de marketing",
  "estrategia de marketing", "plan de crecimiento", "por dónde empiezo a promocionar", "roadmap de
  marketing", o vaya a lanzar/escalar un negocio y necesite el plan maestro. Encaja con la consultora
  IA para PYMEs, la agencia FIFA y los servicios propios. NO la uses para ejecutar una táctica
  concreta (para eso están las skills específicas: `marketing-ads`, `marketing-email-sequence`,
  `marketing-cro`…) ni para un plan de contenido puntual ([[marketing-content-strategy]]).
version: 0.1.0
---

# marketing-plan

> **Adaptación iAmasters OS** — Portada de `coreyhaines31/marketingskills`. Cuerpo original (inglés)
> íntegro debajo. Puente al OS: el doc base `.agents/product-marketing.md` = tu `brand-context/` +
> `context/` (léelos primero; crea/actualiza con [[marketing-product-context]]). Las "Related Skills"
> por nombre pelado (`pricing`, `ads`, `emails`…) son aquí `marketing-<nombre>`.
